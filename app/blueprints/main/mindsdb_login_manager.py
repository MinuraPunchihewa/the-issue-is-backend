...
class MindsDBLoginManager:
    ...
    def login(self, login, password):
        try:
            ...
            # save server object
            self.save_server_for_login(login=login, server=server)
    ...
    def save_server_for_login(self, login, server):
        self.server_objects[login] = server

    def get_server_for_login(self):
        # get login from jwt
        login = get_jwt_identity()

        # get server object
        server = self.server_objects.get(login)
